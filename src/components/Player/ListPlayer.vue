<template>
  <transition
    ...
    @enter="enter"
    @leave="leave">
    <!--<div  v-show="this.$store.getters.isShowListPlayer">-->
...
export default {
  ...
  methods: {
    ...
    enter (el, done) {
      Velocity(el, 'transition.whirlIn', { duration: 560 }, function () {
        done()
      })
    },
    leave (el, done) {
      Velocity(el, 'transition.whirlOut', { duration: 560 }, function () {
        done()
      })
    },
    play () {
    ...
    // 判断歌曲已经是否被收藏过
    isFavorite (song) {
      // 查找收藏列表里的歌曲是否有现在点击要收藏的歌曲
      const result = this.favoriteList.find(function (currentValue) {
        // console.log(currentValue.id, 'id', song.id)
  ...
  watch: {
    ...
    isShowListPlayer (newValue, oldValue) {
      // console.log(newValue, 1, oldValue)
      if (newValue) {
