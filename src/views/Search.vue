<template>
  <div class="search">
    <div class="search-box">
      ...
      <!--用v-model设置双向数据，使外面输入数据时也可以实时更改-->
      <!--使用自定义的节流方法命令-->
...
export default {
  ...
  methods: {
    ...
    delHistory (name) {
      // 过滤器 filter() 方法创建一个新的数组，新数组中的元素是通过检查指定数组中符合条件的所有元素
  ...
  directives: {
    // 节流
    throttle: {
      // 指令的定义  传入控制的元素，还可以传入对象
      // directive方法接收两个参数
      // 第一个参数: 指令的名称  第二个参数: 对象  对象可以用于保存指令执行传递过来的参数
      // bind: 指令被绑定到元素上的时候执行
      // inserted: 绑定指令的元素被添加到父元素上的时候执行    既渲染出来后才使用
  ...
  created () {
    ...
    // 假如本地搜索历史存在就读取
    if (getLocalStorage('searchHistory') === undefined || getLocalStorage() === null) {
      return
    }
