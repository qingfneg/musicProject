<template>
  <div id="app">
    ...
    <Tabbar></Tabbar>
    <Player></Player>
    <!--Singer页面组件和Search页面组件需要经常切换数据量大，且加载后数据一般不会变化，可以用keep-alive来缓存页面-->
...
/*
1.使用预渲染解决SPA应用SEO问题
spa 安装的前提是vue-cli安装是没有问题的  安装：vue add prerender-spa
...
https://www.npmjs.com/package/vue-cli-plugin-prerender-spa
注意点: Router必须使用history模式
安装 vue add prerender-spa
全程输入yes 配置按回车，不要提前配置
安装完会在main.js自动添加
  mounted: () => document.dispatchEvent(new Event('x-app-rendered'))
  也会在vue.config.js添加代码  pluginOptions
在  pluginOptions的renderRoutes配置信息，根据路由地址生成对应的多页面
注意点: 如果需要使用预渲染的插件, 那么Router的模式必须是history模式
还有配置完后会自动修改生成页面对应得组件文件导致不符合eslink规范，需要对每个view里的每个组件elink下
  才打包
  mode: 'history',
  // mode: 'hash',
  pluginOptions: {
...
注意点：要手动将public里的index注释掉，因为会自动添加，就不要手动在添加了
* */
/*
      注意点:
